...
(ns clojure.tools.analyzer.passes.clr.validate
  (:require [clojure.tools.analyzer.ast :refer [prewalk]]
            ...
            [clojure.tools.analyzer.clr.utils :as u :refer [tag-match? try-best-match resolve-hinted-method]])
  ...
  (if-let [handle (-> (env/deref-env) :passes-opts :validate/unresolvable-symbol-handler)]
    ...
    (if-let [resolved-class (u/maybe-class-literal class)]
      (throw (ex-info (str "Cannot find method or field " field " for class "
                           (.Name ^Type resolved-class))                                       ;;; .getName ^Class
    ...
    (if param-tags
      (if-let [m (resolve-hinted-method methods param-tags)]
        ...
        (throw (ex-info (str "param-tags " (pr-str param-tags)
                             ...
                             method " in class " (.Name ^Type  class))                       ;;; .getName ^Class
                        (merge {:class      class
                                :method     method
...
(defmethod -validate :def
  ...
  (when-not (var? (:var ast))
    (throw (ex-info (str "Cannot def " (:name ast) " as it refers to the class "
                         (.FullName ^Type (:var ast)))                                               ;;; .getName ^Class
