...
(defmulti ^Type maybe-class                                                           ;;; ^Class
...
(defn maybe-class-from-string [^String s]
  (or (when-let [maybe-class (and (neg? (.IndexOf s "."))                             ;;; .indexOf
      ...
      (try (RT/classForName s)
           (catch Exception _))))                                                     ;;; ClassNotFoundException
...
(defn maybe-array-class-sym [x]
  (let [sname (name x)]
    (if-let [c (and (= (count sname) 1)
                    (Char/IsDigit (char (first sname)))                         ;;; Character/isDigit
                    (namespace x))]
  ...
  (let [c1 (maybe-class c1)
    ...
    (if (nil? c1)
      ...
      (or
       ...
       (and (primitive? c1)
            (.IsAssignableFrom (box c1) c2))))))         ;;; .isAssignableFrom
